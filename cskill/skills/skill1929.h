...
/* 
19290  "Encantar "19291  "^ffff00Enfeitiçar (Tigre) 

^ffcb4aRequer Forma de Raposa
^ffcb4aAlcance ^ffffff27 ^ffcb4ametros
...
Manipule a energia espiritual para transformar o alvo em um  animal por ^96f5ff6 ^ffffff
segundos. ^96f5ff6^ffffff segundos após conjurar Enfeitiçar e
antes de conjurar o Ataque de Rancor, você irá se teleportar
instantaneamente à posição atual do alvo. A distância máxima é de ^96f5ff30 ^ffffffmetros.

^a6caf0Não tem efeito em alvos não jogadores
^a6caf0Custa ^ffffff50 ^a6caf0de Chi.
^a6caf0Compartilha o mesmo tempo de espera com outras versões desta habilidade."*/ 
...
{ 
    ...
    { 
        ...
        skill->GetVictim ()->SetNewValue3 (1826); //1
        skill->GetVictim ()->SetNewValue4 (1929); //6
        skill->GetVictim ()->SetBarbianJumpToTarget (1); 

        skill->GetVictim()->SetProbability(1.0 * 100);
        skill->GetVictim()->SetTime(7000); //tempo
        skill->GetVictim()->SetAmount(30); //metros
        skill->GetVictim()->SetValue(1); //quantidade max
        skill->GetVictim()->SetShattereddreamMonkey(1);
        return true; 
    } 
#endif 
#ifdef _SKILL_SERVER 
