...
bool SkillStub::ReqSecLevel(Skill * skill ) const
{
	PlayerWrapper * player = skill->GetPlayer();
	unsigned int cur = player->GetRank();
	unsigned int flag = player->GetFlag();
	unsigned int req = rank;
...
bool SkillStub::ReqPreSKill(Skill * skill ) const
{
	PlayerWrapper * player = skill->GetPlayer();
...
bool SkillStub::LearnCondition(Skill * skill ) const
{
	...
	if (!ReqSecLevel(skill))
	{
		return false;
	}

	if (!ReqPreSKill(skill))
	{
		return false;
	}
...
bool SkillStub::Push(Skill *skill) const
{
	PlayerWrapper * player = skill->GetPlayer();
	int next_level = skill->GetLevel();
	if (next_level < MIN_LEVEL) next_level = MIN_LEVEL;
...
int SkillStub::Condition(Skill *skill) const
{
	...
	int newapcost = 0;
	if(skill->GetPlayer()->CheckBuffState(543))
	{
		newapcost = apcost * 1.5;
	}
	else
	{
		// Ap Cost Runa
		int newap = skill->GetPlayer()->GetGlyphApCostInc();
		if(newap > 0)
		{
			//printf("int SkillStub::Condition(Skill *skill) const:: GetId () = %d, newap: %d \n",GetId (), newap);
			newapcost = (apcost - newap);
		}
		else
		{
			// Ap Cost Normal
		...
		//printf("int SkillStub::Condition(Skill *skill) const:: GetId () = %d, newapcost: %d \n",GetId (), newapcost);
	}

	if(skill->GetPlayer()->GetAp()<newapcost)
		return 4;
	...
	if(itemcost > 0 && !skill->GetPlayer()->SetCheckitem(itemcost, itemcostcount))	
		return 10;
	...
	//if(!skill->CanAttackCondition(false))
	if(!skill->CanAttack(false))
		return 12;

	if(skill->GetPlayer ()->GetCls () != 12)
	{
	...
	if ( GetCostShieldEnergy(skill) && !skill->GetPlayer()->UseSheild(GetCostShieldEnergy(skill)) )
		return 14;
...
int SkillStub::ElfCondition(Skill *skill) const
{
	...
	if(skill->GetPlayer()->GetElfmp()<(int)(GetMpcost(skill)))
		return 3;
	//С�����������?:apcost����λ��ϵ������λΪ��ֵ�����۳�Ϊ ��ֵ+ϵ��*(skilllevel-1)
...
bool Skill::CanAttackCondition(bool usearrow) 
{
	if(stub->IsAttack() && !player->CanAttack())
		return false;
	
	
	if ( !player->BugSkillRelease( GetId() , player->GetCls() ) )
	{
		unsigned int i;
		for( i=0; i<stub->restrict_weapons.size(); i++ )
		{
			if( player->IsUsingWeapon(stub->restrict_weapons[i]) /*|| player->IsBuffWeapon(stub->restrict_weapons[i])*/ )
				break;
		}
...
#endif
/*
float Skill::GetPraydistance() 
{
	float range = stub->GetPraydistance(this);			
	return (float) range;	
}

float Skill::GetEffectdistance() 
{
	float range = stub->GetEffectdistance(this);
	return (float) range;	
}
*/
...
int Skill::FirstRun( int & next_interval, int prayspeed )
{
	...
	{
		// Reduz a Canalizacao
		int casting_reduce = GetPlayer()->GetGlyphCastingInc();
		if(casting_reduce > 0)
		{
			time = casting_reduce;
		}
		
		prayspeed = GetPlayer()->FirstRunRelease(player->GetCls(), prayspeed);
...
void Skill::PrepareTalent(PlayerWrapper* player)
{
	...
	t3 = (int)stub->GetTalent3(player);
	t4 = (int)stub->GetTalent4(player);
}
...
int Skill::GetCritAttack()        { return player->GetCritAttack((int)ratio, (int)plus); }
int Skill::GetCritMagicattack()   { return player->GetCritMagicattack((int)ratio,(int)plus); }
