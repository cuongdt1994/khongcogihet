...
DEFINE_SUBSTANCE(filter_InchurtMonkey ,filter,CLASS_INCHURTMONKEY);
...
DEFINE_SUBSTANCE(filter_AdddefencedegreeAtr ,filter,CLASS_ADDDEFENCEDEGREEATR);
DEFINE_SUBSTANCE(filter_AddattackdegreeAtr ,filter,CLASS_ADDATTACKDEGREEATR);
...
DEFINE_SUBSTANCE(filter_SoulbeatbackWf,filter,CLASS_SOULBEATBACKWF);
DEFINE_SUBSTANCE(filter_SoulbeatbackPaladin,filter,CLASS_SOULBEATBACKPALADIN);
...
DEFINE_SUBSTANCE(filter_EnhanceskilldamageBarbian,filter,CLASS_ENHANCESKILLDAMAGEBARBIAN);
DEFINE_SUBSTANCE(filter_EnhanceskilldamagePlayer,filter,CLASS_ENHANCESKILLDAMAGEPLAYER);
...
DEFINE_SUBSTANCE(filter_Deathresetcd2, filter, CLASS_DEATHRESETCD2);
...
DEFINE_SUBSTANCE(filter_Freecooldown ,filter,CLASS_FREECOOLDOWN);
...
DEFINE_SUBSTANCE(filter_Incfarskilldmgreduce ,filter,CLASS_INCFARSKILLDMGREDUCE);
...
DEFINE_SUBSTANCE(filter_SetNewWeapon ,filter,CLASS_SETNEWWEAPONADDON);

DEFINE_SUBSTANCE(filter_SetGlyphRange ,filter,CLASS_GLYPH_RANGE);
DEFINE_SUBSTANCE(filter_SetGlyphRange2 ,filter,CLASS_GLYPH_RANGE2);
DEFINE_SUBSTANCE(filter_SetGlyphCritical ,filter,CLASS_GLYPH_CRITICAL);
DEFINE_SUBSTANCE(filter_SetGlyphCriticalAtr ,filter,CLASS_GLYPH_CRITICALATR);
DEFINE_SUBSTANCE(filter_SetGlyphCritical2 ,filter,CLASS_GLYPH_CRITICAL2);
DEFINE_SUBSTANCE(filter_SetGlyphCritical3 ,filter,CLASS_GLYPH_CRITICAL3);
DEFINE_SUBSTANCE(filter_SetGlyphDamage ,filter,CLASS_GLYPH_DAMAGE);
DEFINE_SUBSTANCE(filter_SetGlyphApCost ,filter,CLASS_GLYPH_APCOST);
DEFINE_SUBSTANCE(filter_SetGlyphApCostMulti ,filter,CLASS_GLYPH_APCOST_MULTI);
DEFINE_SUBSTANCE(filter_SetGlyphCasting ,filter,CLASS_GLYPH_CASTING);
DEFINE_SUBSTANCE(filter_SetGlyphCritical4 ,filter,CLASS_GLYPH_CRITICAL4);

DEFINE_SUBSTANCE(filter_SetApplyDamageforRange ,filter,CLASS_APPLYDAMAGEFORRANGE);
DEFINE_SUBSTANCE(filter_SetShieldDamageReduce ,filter,CLASS_SHIELDDAMAGEREDUCE);
DEFINE_SUBSTANCE(filter_SetShieldDamageReduceBarbian ,filter,CLASS_SHIELDDAMAGEREDUCEBARBIAN);

DEFINE_SUBSTANCE(filter_SetGlyphJumpRollback ,filter,CLASS_GLYPH_JUMPROLLBACK);
DEFINE_SUBSTANCE(filter_SetInvincibleAtr ,filter,CLASS_INVINCIBLEATR);
DEFINE_SUBSTANCE(filter_SetBarbianAssault ,filter,CLASS_BARBIANASSAULT);
DEFINE_SUBSTANCE(filter_SetBarbianJumpToTarget ,filter,CLASS_BARBIANJUMPTOTARGET);
DEFINE_SUBSTANCE(filter_ShieldTransferDebuffBarbian ,filter,CLASS_SHEILDTRANSFERDEBUFFBARBIAN);
DEFINE_SUBSTANCE(filter_SetInvincibleWF ,filter,CLASS_INVINCIBLEWF);
DEFINE_SUBSTANCE(filter_SetWfJumpToTarget ,filter,CLASS_BARBIANJUMPTOTARGETWF);
DEFINE_SUBSTANCE(filter_ContinuousDamageGold ,filter,CLASS_CONTINUOUSDAMAGEWF1);
DEFINE_SUBSTANCE(filter_ContinuousDamageWood ,filter,CLASS_CONTINUOUSDAMAGEWF2);
DEFINE_SUBSTANCE(filter_ContinuousDamageWater ,filter,CLASS_CONTINUOUSDAMAGEWF3);
DEFINE_SUBSTANCE(filter_ContinuousDamageFire ,filter,CLASS_CONTINUOUSDAMAGEWF4);
DEFINE_SUBSTANCE(filter_ContinuousDamageEarth ,filter,CLASS_CONTINUOUSDAMAGEWF5);

DEFINE_SUBSTANCE(filter_Elmodisabled,filter,CLASS_ELMODISABLED);
DEFINE_SUBSTANCE(filter_GlyphMercInvisible,filter,CLASS_GLYPHINVISIBLE);

DEFINE_SUBSTANCE(filter_DebuffGlyphSK1,filter,CLASS_DEBUFFGLYPHSK1);
DEFINE_SUBSTANCE(filter_DebuffGlyphSK2,filter,CLASS_DEBUFFGLYPHSK2);
DEFINE_SUBSTANCE(filter_DebuffGlyphSK3,filter,CLASS_DEBUFFGLYPHSK3);
DEFINE_SUBSTANCE(filter_DebuffGlyphSK4,filter,CLASS_DEBUFFGLYPHSK4);
DEFINE_SUBSTANCE(filter_GlyphCriticalValid,filter,CLASS_FREECLEARDEBUFF);

DEFINE_SUBSTANCE(filter_ImmuneRoar,filter,CLASS_IMMUNEROAR); // ROAR
DEFINE_SUBSTANCE(filter_NochangeselectMob,filter,CLASS_NOCHANGESELECTMOB);
DEFINE_SUBSTANCE(filter_DizzyNoImmune ,filter,CLASS_DIZZYNOIMMUNE);
DEFINE_SUBSTANCE(filter_CantiBlow ,filter,CLASS_CANTIBLOW);
DEFINE_SUBSTANCE(filter_DeathresetcdVoidStep, filter, CLASS_DEATHRESETCDVOIDSTEP);
DEFINE_SUBSTANCE(filter_EnhanceskilldamagePELOTAO, filter, CLASS_ENHANCESKILLDAMAGEPELOTAO);

DEFINE_SUBSTANCE(filter_Dizzyondamage,filter,CLASS_DIZZYONDAMAGE);

DEFINE_SUBSTANCE(filter_SetSwordForm ,filter,CLASS_SETSWORDFORM);
//DEFINE_SUBSTANCE(filter_SetAxeForm ,filter,CLASS_SETAXEFORM);
//DEFINE_SUBSTANCE(filter_SetFistForm ,filter,CLASS_SETFISTFORM);
//DEFINE_SUBSTANCE(filter_SetPoleArmForm ,filter,CLASS_SETPOLEARMFORM);

DEFINE_SUBSTANCE(filter_ImmuneTransfWB, filter, CLASS_IMMUNETRANSFWB);
DEFINE_SUBSTANCE(filter_Adddefencedegree2 ,filter,CLASS_ADDDEFENCEDEGREE2);
DEFINE_SUBSTANCE(filter_Incdefensedegreeondamage ,filter,CLASS_INCDEFENSEDEGREEONDAMAGE);
DEFINE_SUBSTANCE(filter_PhysMagReducwithstackWB ,filter,CLASS_PHYSMAGREDUCWITHSTACKWB);

DEFINE_SUBSTANCE(filter_NewStun, filter, CLASS_NEWSTUN);
DEFINE_SUBSTANCE(filter_ShieldDamageMystic, filter, CLASS_SHIELDDAMAGEMYSTIC);

DEFINE_SUBSTANCE(filter_Anticlearbuf2, filter, CLASS_ANTICLEARBUF2);
DEFINE_SUBSTANCE(filter_ConvertAtaqMtoAtaqF, filter, CLASS_CONVERTATAQMTOATAQF);
DEFINE_SUBSTANCE(filter_ConvertAtaqMtoAtaqF2, filter, CLASS_CONVERTATAQMTOATAQF2);

DEFINE_SUBSTANCE(filter_DecdefenceNew ,filter,CLASS_DECDEFENCENEW);

DEFINE_SUBSTANCE(filter_IncDefenseResitWithDamage ,filter,CLASS_INCDEFENSERESITWITHDAMAGE);
DEFINE_SUBSTANCE(filter_IncDefenseResitWithDamageStart ,filter,CLASS_INCDEFENSERESITWITHDAMAGESTART);

DEFINE_SUBSTANCE(filter_ElementDamage, filter, CLASS_ELEMENTDAMAGE);
DEFINE_SUBSTANCE(filter_NewShapeAndSkills, filter, CLASS_NEWSHAPEANDSKILLS);

DEFINE_SUBSTANCE(filter_guildreducedefensepassive ,filter,CLASS_GUILDREDUCEDEFENSEPASSIVE);

// G17/G18
DEFINE_SUBSTANCE(filter_SetInvisibleForced,filter,CLASS_SETINVISIBLEFORCED);
DEFINE_SUBSTANCE(filter_SetForceTransform,filter,CLASS_SETFORCETRANSFORM);
DEFINE_SUBSTANCE(filter_SetRecoveryApperSec,filter,CLASS_SETRECOVERYAPPERSEC);
DEFINE_SUBSTANCE(filter_SetAdjustDamageSendRecv,filter,CLASS_SETADJUSTDAMAGESENDRECV);
DEFINE_SUBSTANCE(filter_SetAdjustHPFromMP,filter,CLASS_SETADJUSTHPFROMMP);
DEFINE_SUBSTANCE(filter_SetIncDamageReducePerc,filter,CLASS_SETINCDAMAGEREDUCEPERC);
DEFINE_SUBSTANCE(filter_SetIncImmuneNegativeEffect,filter,CLASS_SETIIMMUNENEGATIVEEFFECT);
DEFINE_SUBSTANCE(filter_SetReduceNormalDamage,filter,CLASS_SETRECUCENORMALDAMAGE);
DEFINE_SUBSTANCE(filter_SetSpeedNoCombat,filter,CLASS_SETSPEEDNOCOMBAT);
DEFINE_SUBSTANCE(filter_SetAdjustDamageSendRecv2,filter,CLASS_SETADJUSTDAMAGESENDRECV2);
DEFINE_SUBSTANCE(filter_SetPlusDamageFromHP,filter,CLASS_SETPLUSDAAMAGEFROMHP);
...
void filter_Tigerform::OnAttach()
{
	...
	{
		_parent.Heal(_parent.GetExtendProp ().max_hp * _inchp);
	}
...
void filter_Foxform::OnRelease()
{
	...
	_parent.UpdateAll();
	_parent.SendClientAttackData();
	//_parent.RemoveFilter(FILTER_CONVERTATAQMTOATAQF);
	_parent.RemoveTeamVisibleState(HSTATE_FOXFORM);
}

void filter_ConvertAtaqMtoAtaqF2::OnAttach()
{
	//_parent.ImpairScaleMagicDamage(100);
	//_parent.EnhanceScaleDamage(100);
	_parent.ConvertPercDamageMtoF(1, _ratio);
	_parent.UpdateAttackData();
	_parent.UpdateMagicData();
	_parent.UpdateAll();
	_parent.SendClientAttackData();
	_parent.InsertTeamVisibleState(HSTATE_579,-1);
}

void filter_ConvertAtaqMtoAtaqF2::OnRelease()
{
	_parent.ConvertPercDamageMtoF(2, _ratio);
	//_parent.EnhanceScaleMagicDamage(100);
	//_parent.ImpairScaleDamage(100);
	_parent.UpdateAttackData();
	_parent.UpdateMagicData();
	_parent.UpdateAll();
	_parent.SendClientAttackData();
...
void filter_Activateskill::TranslateSendAttack(const XID & target, attack_msg & msg)
{
	...
	if(sid && msg.attached_skill.skill==0 && !(msg._attack_state&AT_STATE_AURA_AND_RETORT))	//Reverse Shock and Aura damage are not processed
	{
...
void filter_Activateskill2::TranslateSendAttack(const XID & target, attack_msg & msg)
{
	...
	if(sid && !(msg._attack_state&AT_STATE_AURA_AND_RETORT))	//Reverse Shock and Aura damage are not processed
	{
		SkillKeeper skill = Skill::Create(sid);
		if(!skill)
			return;
...
{
        #define ARS_XID 7050
		if(msg._attack_state & AT_STATE_AURA_AND_RETORT) return;          //Ignore Reverse Aura etc.        
        if(rand()%100 >= _trigger_prob)	return;         //judgment probability
		...
		if(!_period || _period + _lasttime <= now)
		{
			...
			if(!skill)
					return;
			if(skill->GetRange().IsSelf()) //buff em si mesmo
			{
				PlayerWrapper   w_player(_parent,NULL,skill);
				skill->SetVictim(&w_player);
				skill->SetPerformerid(_parent.GetSelfID());
