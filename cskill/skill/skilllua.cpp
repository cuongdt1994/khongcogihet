...
double LuaSkill::GetT3(GNET::Skill * skill, int id)
{
	double res = -0;
	long long FUNC, STATE, SKILL, REID;
	if( skill && id > 0 && data.cskill.GET_SKILL(id, FUNC, STATE, SKILL, REID) && (FUNC & FUNC_SKILL) && (SKILL & SKILL_GETTALENT3) )
	{
		char EventName[40];
		sprintf(EventName, "Skill%dStub__GetT3", id);
		res = Event<double>(EventName,(long long)skill);
	}
	return res;
}	

double LuaSkill::GetT4(GNET::Skill * skill, int id)
{
	double res = -0;
	long long FUNC, STATE, SKILL, REID;
	if( skill && id > 0 && data.cskill.GET_SKILL(id, FUNC, STATE, SKILL, REID) && (FUNC & FUNC_SKILL) && (SKILL & SKILL_GETTALENT4) )
	{
		char EventName[40];
		sprintf(EventName, "Skill%dStub__GetT4", id);
		res = Event<double>(EventName,(long long)skill);
	}
	return res;
}	
