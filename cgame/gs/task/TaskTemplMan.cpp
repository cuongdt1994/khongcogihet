...
void ATaskTemplMan::CheckAutoDelv(TaskInterface* pTask)
{
	...
	for (; it != m_AutoDelvMap.end(); ++it)
	{
		...
		if (pTempl->CheckPrerequisite(pTask, pLst, ulCurTime) == 0)
		{
			...
			_notify_svr(pTask, TASK_CLT_NOTIFY_AUTO_DELV, static_cast<unsigned short>(pTempl->m_ID), static_cast<unsigned short>(pTempl->x_IDSpace));
		}
	}
}
...
void ATaskTemplMan::CheckSpecialAwardMask(TaskInterface* pTask)
{
	...
	for (int i = 0;i < NUM_SPECIAL_AWARD;i++)
	{
		if (m_SpecialAward.special_mask & (1 << i))
		{
			...
			{
				_notify_svr(pTask, TASK_CLT_NOTIFY_SPECIAL_AWARD_MASK, static_cast<unsigned short>(pTempl->m_ID), static_cast<unsigned short>(pTempl->x_IDSpace));
			}
		}
	}
}
void ATaskTemplMan::ManualTrigTask(TaskInterface* pTask, unsigned int ulTask, unsigned int ulSpace)
{
	_notify_svr(pTask, TASK_CLT_NOTIFY_MANUAL_TRIG, static_cast<unsigned short>(ulTask), ulSpace);
}

void ATaskTemplMan::ForceGiveUpTask(TaskInterface* pTask, unsigned int ulTask, unsigned int ulSpace)
{
	_notify_svr(pTask, TASK_CLT_NOTIFY_FORCE_GIVEUP, static_cast<unsigned short>(ulTask), ulSpace);
}

void ATaskTemplMan::ForceRemoveFinishTask(TaskInterface* pTask, unsigned int ulTask, unsigned int ulSpace)
{
#ifdef _DEBUG
	_notify_svr(pTask, TASK_CLT_NOTIFY_RM_FINISH_TASK, static_cast<unsigned short>(ulTask), ulSpace);
#endif
}
...
void ATaskTemplMan::OnDynTasksTimeMark(TaskInterface* pTask, unsigned int ulTimeMark, unsigned short version)
{
	...
	if (m_ulDynTasksTimeMark == ulTimeMark && LoadDynTasksFromPack(m_szDynPackPath))
	{
		...
		_notify_svr(pTask, TASK_CLT_NOTIFY_DYN_DATA, 0, 0);
}
...
inline void _SendPlayerStorageData(TaskInterface* pTask, StorageTaskList* pLst)
{
	...
	reinterpret_cast<task_notify_base*>(buf)->space = 0;
...
bool ATaskTemplMan::PickTaskByProbability(TaskInterface* pTask, StorageTaskList* pStorageLst, int idStorage, unsigned int ulCurTime, const NPC_TASK_OUT_SERVICE* pEssence)
{
	...
	unsigned int task_array_size = SIZE_OF_ARRAY(pEssence->tasks);
	// 填充稀有任务容器和普通任务容器
	for (unsigned int j = 0; j < task_array_size; j++)
	{
		unsigned int idTask = pEssence->tasks[j].id;
		if (idTask == 0)
			break;
				...
				if (distance <= dist2) {
					if (distance <= pTemp->m_ucZoneSide / 2) {
						if (pTemp->IsValidState())
						{
							...
							_notify_svr(pTask, TASK_CLT_NOTIFY_CHECK_FINISH, (unsigned short)pTemp->GetID(), (unsigned short)pTemp->GetSpaceID());
						}
					}
...
bool ATaskTemplMan::InitStorageTask()
{
	...
	while (id) {
		if (dt == DT_NPC_TASK_OUT_SERVICE) {
			...
			unsigned int sTasks = SIZE_OF_ARRAY(pData->tasks);
			for (unsigned int i = 0; i < sTasks; i++) {
				if (pData->tasks[i].id) {
					m_StorageTaskMap[pData->tasks[i].id] = pData->storage_id;
				}
			}
		}
		id = m_pEleDataMan->get_next_data_id(ID_SPACE_ESSENCE, dt);
...
void mount_task_out_service(const ATaskTempl* task, NPC_TASK_OUT_SERVICE* svr)
{
	...
	for (unsigned int i = 0; i < SIZE_OF_ARRAY(svr->tasks); i++)
	{
		if (svr->tasks[i].id) continue;
		svr->tasks[i].id = task->GetID();
		break;
	}
...
void ATaskTemplMan::UpdateDynDataNPCService()
{
	...
	memset(deliver->tasks, 0, sizeof(deliver->tasks));
	memset(service->tasks, 0, sizeof(service->tasks));
