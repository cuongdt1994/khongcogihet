...
DEFINE_SUBSTANCE(gmatter_money2_imp,gmatter_item_base_imp,CLS_MATTER_MONEY2_IMP)
...
void 
gmatter_money2_imp::OnPickup(const XID & who,int team_id, bool is_team)
{
	MSG  msg;
	if(team_id > 0)
	{
		BuildMessage(msg,GM_MSG_PICKUP_TEAM_MONEY2,XID(GM_TYPE_PLAYER,team_id),_parent->ID,
				_parent->pos,_money,&_drop_user,sizeof(_drop_user));
	}
	else
	{
		BuildMessage(msg,GM_MSG_PICKUP_MONEY2,who,_parent->ID,
				_parent->pos,_money,&_drop_user,sizeof(_drop_user));
	}
	_plane->PostLazyMessage(msg);
}
...
int 
gmatter_money2_imp::MessageHandler(world * pPlane ,const MSG & msg)
{
	switch(msg.message)
	{
		case GM_MSG_PICKUP:
...
void DropMoney2Item(world * pPlane, const A3DVECTOR & pos, unsigned int amount,const XID &owner,int owner_team, int seq,int drop_id)
{
	gmatter * matter = pPlane->AllocMatter();
	if(matter == NULL) return ;
	matter->SetActive();
	...
	matter->ID.id= MERGE_ID<gmatter>(MKOBJID(world_manager::GetWorldIndex(),pPlane->GetMatterIndex(matter)));
	matter->SetDirUp(0,0,abase::Rand(0,255));
	gmatter_money2_imp *imp = new gmatter_money2_imp(amount);
	imp->SetOwner(owner,owner_team,seq);
	imp->SetDrop(drop_id);
	imp->Init(pPlane,matter);
	matter->imp = imp;
