...
#include "getluastorage.hrp"
#include "setluastorage.hrp"

#include "getnewtrashbox.hrp"
#include "setnewtrashbox.hrp"

#include "setnewroledetail.hrp"
#include "getnewroledetail.hrp"
...
namespace GDB
{
	...
	{
		if(corral.count == 0) return;
		free(corral.list);
		corral.list = NULL;
		corral.count = 0;
	}

	void get_itemlist(const GNET::GRoleInventoryVector & backpack, itemlist & list)
	{
	...
	void set_itemlist(GNET::GRoleInventoryVector & backpack, const itemlist & list)
	{
		if(list.count == 0) return;
						...
						case GROLE_STATUS_EXTRAPROP_DAILY_SIGN_IN:
							{
								...
								>> info.daily_signin.awarded_times2
						...
						case GROLE_STATUS_EXTRAPROP_GIFTCARD_HALF_REDEEM:
							{
								...
								>> cardnumber;
								if(cardnumber.size() != sizeof(info.giftcard_redeem.cardnumber)) throw Marshal::Exception();
						...
						case GROLE_STATUS_EXTRAPROP_DAILY_SIGN_IN:
							{
								...
								info.daily_signin.awarded_times2 = 0;
	...
	inline void  set_extraprop(GNET::Octets& odata,const base_info& info, const vecdata & data)
	{
		...
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_TOUCH_HALF_TRADE]
		...
		extraprop.data[GROLE_STATUS_EXTRAPROP_DAILY_SIGN_IN]
			= Marshal::OctetsStream()
				<< info.daily_signin.update_time
				<< info.daily_signin.month_calendar
				<< info.daily_signin.curr_year_data
		...
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_GIFTCARD_HALF_REDEEM]
				= Marshal::OctetsStream()
					<< info.giftcard_redeem.state
					<< info.giftcard_redeem.type
					<< info.giftcard_redeem.parenttype
		...
		extraprop.data[GROLE_STATUS_EXTRAPROP_LEADERSHIP]
			= Marshal::OctetsStream() << info.leadership;
		
		if(data.generalcard_collection.size)
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_GENERALCARD_COLLECTION]
				= Octets(data.generalcard_collection.data, data.generalcard_collection.size);
		}

		if(data.fatering_data.size)
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_FATERING]
				= Octets(data.fatering_data.data, data.fatering_data.size);
		}
		
		if(data.clock_data.size)
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_CLOCK_DATA]
				= Octets(data.clock_data.data, data.clock_data.size);
		}

		if(data.rand_mall_data.size)
		{
			extraprop.data[GROLE_STATUS_EXTRAPROP_RAND_MALL_DATA]
				= Octets(data.rand_mall_data.data, data.rand_mall_data.size);
		}
	...
	bool Role2Info(GNET::GRoleDetail * pRole,base_info& info, vecdata& data,int data_mask,
			const GNET::GPetCorral& corral, const GNET::GRoleStatusExtraProp & extraprop)
	{
		...
		//info.storesize4 = pRole->storehouse.size4;
		//info.storesize5 = pRole->storehouse.size5;
		...
		if(data_mask & GET_STOREHOUSE)
		{
			...
			//get_itemlist(pRole->storehouse.glyph_item,data.trash_box4);
			//get_itemlist(pRole->storehouse.fly_item,data.trash_box5);
			info.trash_money = pRole->storehouse.money;
		}
		info.userstore_active = true;//Ĭ�����һ��ȡ�˺Ųֿ�����
	...
	void ReleaseAllInventory(vecdata & data)
	{
		...
		//release_itemlist(data.trash_box4);
		//release_itemlist(data.trash_box5);
	...
	void Info2Role(GNET::GRoleDetail * pRole,const base_info & info,const vecdata & data)
	{
		...
		//pRole->storehouse.size4 = info.storesize4;
		//pRole->storehouse.size5 = info.storesize5;
		...
		if(info.trashbox_active)
		{
			...
			//set_itemlist(pRole->storehouse.glyph_item,data.trash_box4);
			//set_itemlist(pRole->storehouse.fly_item,data.trash_box5);
		...
		//int res = asm_data_save( info.id, data.asm_data );// �����ѧߧ֧ߧڧ�
	}
	...
	bool OnGetLuaData(int type, int key, OnGetLuaDataResult* callback )
	{
		GetLuaStorageArg arg( key , type );
		Rpc *rpc = Rpc::Call(RPC_GETLUASTORAGE, &arg);
		((GetLuaStorage *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}
	
	bool OnSetLuaData(int type, int key, void * data, unsigned int size, OnSetLuaDataResult * callback )
	{
		SetLuaStorageArg arg( type );
		arg.value.key = key;
		arg.value.data.replace(data,size);
		Rpc *rpc = Rpc::Call(RPC_SETLUASTORAGE, &arg);
		((SetLuaStorage *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}	

	bool OnGetNewTrashBox(int roleid, OnGetNewTrashBoxResult * callback )
	{
		GetNewTrashBoxArg arg;
		arg.roleid = roleid;
		Rpc *rpc = Rpc::Call(RPC_GETNEWTRASHBOX, &arg);
		((GetNewTrashBox *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}

	bool OnSetNewTrashBox(int roleid, itemlist & box4, itemlist & box5, itemlist & box6, itemlist & box7, OnSetNewTrashBoxResult * callback )
	{
		GNET::GRoleInventoryVector cbox4; cbox4.clear();
		GNET::GRoleInventoryVector cbox5; cbox5.clear();
		GNET::GRoleInventoryVector cbox6; cbox6.clear();
		GNET::GRoleInventoryVector cbox7; cbox7.clear();
		
		itemlist_to_inventory(cbox4, box4);
		itemlist_to_inventory(cbox5, box5);
		itemlist_to_inventory(cbox6, box6);
		itemlist_to_inventory(cbox7, box7);

		SetNewTrashBoxArg arg;
		arg.value.roleid = roleid;
		
		arg.value.size4 = cbox4.size();
		arg.value.box4 = cbox4;

		arg.value.size5 = cbox5.size();
		arg.value.box5 = cbox5;

		arg.value.size6 = cbox6.size();
		arg.value.box6 = cbox6;

		arg.value.size7 = cbox7.size();
		arg.value.box7 = cbox7;		
		
		Rpc *rpc = Rpc::Call(RPC_SETNEWTRASHBOX, &arg);
		((SetNewTrashBox *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}
	...
	inline void NewLoad(char & o , const char & i) 				{ o=i; }
	inline void NewLoad(short & o , const short & i) 			{ o=i; }
	inline void NewLoad(int & o , const int & i) 				{ o=i; }
	inline void NewLoad(long long & o , const long long & i) 	{ o=i; }
	inline void NewLoad(DBStream & o , const GNET::Octets & i)	{ i.size() ? o.replace( i.begin(), i.size() ) : o.clear(); }
	
	bool LoadNewRoleDetail(const GNET::GNewRoleDetail & pRole, vecdata_new & data)
	{
		
		NewLoad( data.roleid                 , pRole.roleid                 );
		NewLoad( data.pk_time                , pRole.pk_time                );
		NewLoad( data.pk_status              , pRole.pk_status              );
		NewLoad( data.money_silver           , pRole.money_silver           );
		NewLoad( data.color_name             , pRole.color_name             );
		NewLoad( data.pvp_rank               , pRole.pvp_rank               );
		NewLoad( data.pvp_rank_exp           , pRole.pvp_rank_exp           );
		NewLoad( data.player_kill            , pRole.player_kill            );
		NewLoad( data.monster_kill           , pRole.monster_kill           );
		NewLoad( data.player_death           , pRole.player_death           );
		NewLoad( data.monster_death          , pRole.monster_death          );
		NewLoad( data.diary_exp              , pRole.diary_exp              );
		NewLoad( data.realm_day_verify       , pRole.realm_day_verify       );
		NewLoad( data.verify_itens_valid     , pRole.verify_itens_valid     );
		NewLoad( data.has_astrolabe_lock     , pRole.has_astrolabe_lock     );
		NewLoad( data.enabled_fashion_weapon , pRole.enabled_fashion_weapon );
		NewLoad( data.double_factor_exp      , pRole.double_factor_exp      );
		NewLoad( data.double_factor_sp       , pRole.double_factor_sp       );
		NewLoad( data.double_factor_realm    , pRole.double_factor_realm    );
		NewLoad( data.autoswap               , pRole.autoswap               );
		NewLoad( data.skillsender            , pRole.skillsender            );
		NewLoad( data.glyph                  , pRole.glyph                  );
		NewLoad( data.carrier                , pRole.carrier                );
		NewLoad( data.repository             , pRole.repository             );
		NewLoad( data.treasure               , pRole.treasure               );
		NewLoad( data.lottery                , pRole.lottery                );
		NewLoad( data.treasure_items         , pRole.treasure_items         );
		NewLoad( data.lib_items              , pRole.lib_items              );
		NewLoad( data.pet_skill_temp         , pRole.pet_skill_temp         );
		NewLoad( data.pet_skin               , pRole.pet_skin               );
		NewLoad( data.passwd_safe            , pRole.passwd_safe            );
		NewLoad( data.day_world_points       , pRole.day_world_points       );
	...
	bool OnLoadNewRoleDetail(int roleid, OnLoadNewRoleDetail_Re * callback )
	{
		GetNewRoleDetailArg arg;
		arg.roleid = roleid;
		Rpc *rpc = Rpc::Call(RPC_GETNEWROLEDETAIL, arg);
		((GetNewRoleDetail *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}
	
	inline void NewSave(const char & i , char & o) 				{ o=i; }
	inline void NewSave(const short & i , short & o) 			{ o=i; }
	inline void NewSave(const int & i , int & o) 				{ o=i; }
	inline void NewSave(const long long & i , long long & o) 	{ o=i; }
	inline void NewSave(const DBStream & i , GNET::Octets & o)	{ i.size() ? o.replace( i.begin(), i.size() ) : o.clear(); }
	
	bool SaveNewRoleDetail(GNET::GNewRoleDetail & pRole, const vecdata_new & data)
	{
		
		NewSave( data.roleid                 , pRole.roleid                 );
		NewSave( data.pk_time                , pRole.pk_time                );
		NewSave( data.pk_status              , pRole.pk_status              );
		NewSave( data.money_silver           , pRole.money_silver           );
		NewSave( data.color_name             , pRole.color_name             );
		NewSave( data.pvp_rank               , pRole.pvp_rank               );
		NewSave( data.pvp_rank_exp           , pRole.pvp_rank_exp           );
		NewSave( data.player_kill            , pRole.player_kill            );
		NewSave( data.monster_kill           , pRole.monster_kill           );
		NewSave( data.player_death           , pRole.player_death           );
		NewSave( data.monster_death          , pRole.monster_death          );
		NewSave( data.diary_exp              , pRole.diary_exp              );
		NewSave( data.realm_day_verify       , pRole.realm_day_verify       );
		NewSave( data.verify_itens_valid     , pRole.verify_itens_valid     );
		NewSave( data.has_astrolabe_lock     , pRole.has_astrolabe_lock     );
		NewSave( data.enabled_fashion_weapon , pRole.enabled_fashion_weapon );
		NewSave( data.double_factor_exp      , pRole.double_factor_exp      );
		NewSave( data.double_factor_sp       , pRole.double_factor_sp       );
		NewSave( data.double_factor_realm    , pRole.double_factor_realm    );
		NewSave( data.autoswap               , pRole.autoswap               );
		NewSave( data.skillsender            , pRole.skillsender            );
		NewSave( data.glyph                  , pRole.glyph                  );
		NewSave( data.carrier                , pRole.carrier                );
		NewSave( data.repository             , pRole.repository             );
		NewSave( data.treasure               , pRole.treasure               );
		NewSave( data.lottery                , pRole.lottery                );
		NewSave( data.treasure_items         , pRole.treasure_items         );
		NewSave( data.lib_items              , pRole.lib_items              );
		NewSave( data.pet_skill_temp         , pRole.pet_skill_temp         );
		NewSave( data.pet_skin               , pRole.pet_skin               );
		NewSave( data.passwd_safe            , pRole.passwd_safe            );
		NewSave( data.day_world_points       , pRole.day_world_points       );
	...
	bool OnSaveNewRoleDetail(int roleid, const vecdata_new & data, OnSaveNewRoleDetail_Re * callback )
	{
		SetNewRoleDetailArg arg;
		arg.roleid = roleid;
		SaveNewRoleDetail( arg.detail, data );
		Rpc *rpc = Rpc::Call(RPC_SETNEWROLEDETAIL, arg);
		((SetNewRoleDetail *)rpc)->_callback = callback;
		bool success = GamedbClient::GetInstance()->SendProtocol(*rpc);
		if(success) PollIO::WakeUp();
		return success;
	}
