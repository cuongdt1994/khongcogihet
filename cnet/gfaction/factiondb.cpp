...
//		ignore_announce_space = ig_asp;
	...
	#define CONTRIB_EXPEL_LIMIT 2000000000
	...
	{
		if(nick.size()>32 || Matcher::GetInstance()->Match((char*)nick.begin(),nick.size())!=0)
			return ERR_FC_INVALIDNAME;
	...
	void Factiondb::GetAlliance(int fid,  int * buf)
	{
		Thread::RWLock::RDScoped l(locker);
		Map::iterator it = factions.find(fid);
		if(!IsReady(fid, it)) return;
		FactionDetailInfo* pf = it->second;
		for(size_t i=0; i<pf->info.alliance.size() && i < 64; i++)
		buf[i] = pf->info.alliance[i].fid;
	}
